...
PRETRAINED_VOCAB_ARCHIVE_MAP = {
    ...
    'gbert-large': 'https://huggingface.co/deepset/gbert-large/resolve/main/vocab.txt',
    'deepset/gbert-base': 'https://huggingface.co/deepset/gbert-base/resolve/main/vocab.txt',
    'GroNLP/bert-base-dutch-cased': 'https://huggingface.co/GroNLP/bert-base-dutch-cased/resolve/main/vocab.txt',
...
PRETRAINED_VOCAB_POSITIONAL_EMBEDDINGS_SIZE_MAP = {
    ...
    'gbert-large': 512,
    'deepset/gbert-base': 512,
    'GroNLP/bert-base-dutch-cased': 512,
...
def load_vocab(vocab_file):
    ...
    #     if token[0] == '{': # Is json-file
    #         voc = json.loads(token)
    ...
    with open(vocab_file, "r", encoding="utf-8") as reader:
        ...
        if first_line.startswith("{"):  # Prüfe, ob JSON
            reader.seek(0)  # Zurück zum Anfang
            voc = json.load(reader)  # Direkt als JSON einlesen
            vocab = {str(k): int(v) for k, v in voc.items()}  # Sicherstellen, dass Keys Strings sind
        else:  # TXT-Datei
            reader.seek(0)  # Zurück zum Anfang
            for index, token in enumerate(reader):
                vocab[token.strip()] = index  # Token als Key, Index als Value
...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_file, do_lower_case=True, max_len=None, do_basic_tokenize=True, do_roberta_tokenize=False,
        ...
        self.do_roberta_tokenize = do_roberta_tokenize
        if do_roberta_tokenize:
            self.roberta_tokenizer = CamembertTokenizer('almanach/camembert-base')
    ...
    def tokenize(self, text):
        ...
        split_tokens = []
        if self.do_roberta_tokenize:
            split_tokens = self.roberta_tokenizer.tokenize(text)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        """
        kwargs['do_roberta_tokenize'] = True if pretrained_model_name_or_path == 'almanach/camembert-base' else False
