...
parser.add_argument("--evaluation_data_1", "-e1", type=str, default=None, help="Path to evaluation data")
...
def extract_output_as_list(output_raw, task):
    try:
        ...
        if task == 'tasd':
            ...
            output_label = list(set(output_label))
        elif task == 'acd':
            output_label = [_tuple[0] for _tuple in output_label]
            output_label = list(set(output_label))
    except:
        output_label = []
    return output_label
...
for path in [ev_path for ev_path in eval_paths if ev_path is not None]:
